...
import exercise_generation.WeightedGraph;
...
public class Visualiser {
	...
	public static FileWriter drawGraph(FileWriter f, Dijkstra d) {
		try {
			...
			for (int i = 1; i <= n; i++) {
				f.write("\\node[vertex] (" + Integer.toString(i) + ") at ({360/" + Integer.toString(n) + " * ("
						+ Integer.toString(i - 1) + " )}:\\radius) {$v_{" + Integer.toString(i) + "}$};\r\n");
			}
			for (int i = 0; i < n; i++) {
				LinkedList<Edge> list = d.getGraph().dirAdjacencylist[i];
				for (int j = 0; j < list.size(); j++) {
					f.write("\\path[edge] (" + Integer.toString(i + 1) + ") -- node[weight] {$"
							+ Integer.toString(list.get(j).weight) + "$} (" + Integer.toString(list.get(j).end + 1)
							+ ");\r\n");
				}
			}
		} catch (IOException e) {
	...
	public static void generateGraphSolution(Dijkstra d, String filename, boolean pdf) {
		try {
			...
			for (int i = 2; i <= d.getVertices(); i++) {
				f.write("{$v_{" + Integer.toString(i) + "}$} & $" + d.getShortestPaths().get(i - 1) + "$ & "
	...
	public static void generateFullGraphSolution(Dijkstra d, String filename, boolean pdf) {
		try {
			...
			for (int c = 1; c < d.getVertices() + 1; c++) {
				...
				for (int i = 2; i <= d.getVertices(); i++) {
					f.write("{$v_{" + Integer.toString(i) + "}$} & $" + paths[i - 1] + "$ & " + stringDistances[i - 1]
							+ calculations[i - 1] + "\\\\ \n");
				}
				f.write("\\hline\n");
				f.write("\\end{tabular}\n");
				...
				for (Integer val : S) {
					f.write("\\path node[selected vertex] at (" + Integer.toString(val) + ") {$v_{"
				...
				f.write("\\begin{center}\n");
				f.write("Vertices explored from $v_{" + Integer.toString(prev) + "}$. Next vertex is $v_{"
						+ Integer.toString(curr) + "}$. Edge relaxations shown in \\textbf{bold}.\n");
				f.write("\\end{center}\n");
					...
					if (e.end != 0) {
						if (distances[e.end] != 0) {
							if (distances[e.end] > distances[e.start] + e.weight) {
								...
								calculations[e.end] = " = min\\{" + distances[e.end] + ", " + distances[e.start] + "+"
										+ e.weight + "\\}";
							}
						} else {
	...
	public static void main(String[] args) throws IOException {
		...
		if (algorithms.get("(1) Dijkstra")) {
			for (int i = 0; i < dijkstraLimit; i++) {
				...
				if (fullSol) {
					generateFullGraphSolution(test, "solutions/tex/dijkstra_answer" + Integer.toString(i + 1) + ".tex",
							pdf);
				}
			}
		}
